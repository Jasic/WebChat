...
$.widget("ui.dialog", {
	_init: function() {
		...
		var self = this,
			...
			uiDialogTitlebar = (this.uiDialogTitlebar = $('<div/>'))
				...
				.append('<a href="#" class="ui-dialog-titlebar-close"></a>')
	...
	close: function() {
		...
		this._isOpen = false;
		this._closeAfter(this.options.closeAfterFun);
	},
	
	_closeAfter: function(callback){
		eval(callback);
	},
	...
	open: function() {
		...
		if (this.uiDialog.next().length && this.uiDialog.get(0).parentNode != document.body) {
	...
	_size: function() {
		...
		content.height(container.height() - titlebar.outerHeight() - tbMargin-this.uiDialogButtonPane.outerHeight());
		content.width(container.width() - lrMargin -21);
	}
});
