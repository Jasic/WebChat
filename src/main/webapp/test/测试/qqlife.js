	...
	_preventWXPageScroll = function(){
		...
		document.addEventListener('touchmove',function(e) {
			...
			while (tgt.parentNode){
				if (tgt.className.indexOf('touchSlider')>-1){
					scrollable = true;
					break;
				}
				tgt = tgt.parentNode;
		...
		/*
		var w_directionLocked = false;
		var w_dragStart = null;
		var e_move = function(e){
			
			var absDistX, 
				absDistY,
				deltaX = e.touches[0].pageX - w_dragStart.x,
				deltaY = e.touches[0].pageY - w_dragStart.y;
			if (w_directionLocked === "y") {
                return true;
            } else {
                if (w_directionLocked === "x") {
                    e.preventDefault();
                    return false;
                } else {
		...
		var e_end = function(e){
			window.removeEventListener('touchmove', e_move, false);
			window.removeEventListener('touchend', e_end, false);
			window.removeEventListener('touchcancel', e_end, false);
		};
		window.addEventListener('touchstart', function(e){
			...
			window.addEventListener('touchmove', e_move, false);
			window.addEventListener('touchend', e_end, false);
			window.addEventListener('touchcancel', e_end, false);
		}, false);
		*/
